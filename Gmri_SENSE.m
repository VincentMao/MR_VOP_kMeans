...
% function handle for partial parallel imaging By Vincent Mao 7/7/2016
arg.pforward = @Gmri_pforward;
arg.pback = @Gmri_pback;
arg.backall = @Gmri_backall;
...
% Gmri_pforward(): y = G * x
%
function y = Gmri_pforward(arg, x, coils)
...
if isempty(arg.zmap)
  if isempty(arg.sens)
    ...
    for cc=1:Nr
      idcc = coils(cc);
      y((cc-1)*Nt+1:cc*Nt) = arg.Gnufft * (arg.sens(:,idcc) .* x);
    end
  end
else % approximation
  if isempty(arg.sens)
...
%		y = y + arg.B(:,ll) .* (arg.Gnufft * (arg.Ct(:,ll) .* x));
    ...
    for ll=1:arg.L
      for cc=1:Nr
        idcc = coils(cc);
        tmp = repmat(arg.sens(:,idcc).*arg.Ct(:,ll), [1 nc]) .*x;
        tmp = arg.Gnufft * tmp;
        tmp = repmat(arg.B(:,ll), [1 nc]) .* tmp;
        y((cc-1)*Nt+1:cc*Nt) = y((cc-1)*Nt+1:cc*Nt) + tmp;
      end
...
transform = zeros(Nr*Nt, 1);
for cc = 1: Nr
    idcc = coils(cc);
    transform((cc-1)*Nt+1:cc*Nt) = arg.basis.transform((idcc-1)*Nt+1:idcc*Nt);
end
y = y .* double(repmat(transform, [1 ncol(y)])); % [nd, *nc]
...
nc = ncol(y);
Nt = arg.dim(1);
y = y .* double(repmat(conj(arg.basis.transform), [1 nc]));
...
if isempty(arg.zmap)
  if isempty(arg.sens)
    ...
    x = 0;
    for cc=1:arg.Nc
      x = x + conj(arg.sens(:,cc)) .* (arg.Gnufft' * y((cc-1)*Nt+1:cc*Nt));
    end
  end
else % approximation
  if isempty(arg.sens)
...
%		x = x + conj(arg.Ct(:,ll)) .* (arg.Gnufft' * (conj(arg.B(:,ll)) .* y));
    ...
    for ll=1:arg.L
      for cc=1:arg.Nc
        tmp = repmat(conj(arg.B(:,ll)), [1 nc]) .* y((cc-1)*Nt+1:cc*Nt);
        tmp = arg.Gnufft' * tmp;
...
%
% Gmri_backall(): x = G' * y
% full adjoint ("back-projection"), change to fit the YALMIP 
% By Vincent Mao  -- 07/08/18 
%
function x = Gmri_backall(arg, y)
nc = ncol(y);
Nt = arg.dim(1);
y = y .* double(repmat(conj(arg.basis.transform), [1 nc]));
...
if isempty(arg.zmap)
  if isempty(arg.sens)
    ...
    nufft_ob = arg.Gnufft';
    for cc=1:arg.Nc
      dims = size(y((cc-1)*Nt+1:cc*Nt));
      Lprod = prod(dims(2:end));
      Kd = nufft_ob.arg.st.Kd;
      Nd = nufft_ob.arg.st.Nd;
      Xk_all = nufft_ob.arg.st.p.arg.G' * y((cc-1)*Nt+1:cc*Nt);
      % coil_ob = zeros(prod(Kd), Lprod);			% [*Kd *L]
      for ll=1:Lprod
         Xk = reshape(Xk_all(:,ll), [Kd 1]);	% [(Kd)]
         % Use ifft to implement 2d inverse fourier transform
         if ll == 1
            coil_ob = prod(Kd) * col(ifft_1dim(ifft_1dim(Xk,[],1),[],2)); % scale factor!
         else
            coil_ob = [coil_ob prod(Kd) * col(ifft_1dim(ifft_1dim(Xk,[],1),[],2))];
         end
      end
      coil_ob = reshape(coil_ob, [Kd Lprod]);			% [(Kd) *L]

      % eliminate zero padding from ends  fix: need generic method
      switch length(Nd)
      case 1
      coil_ob = coil_ob(1:Nd(1),:); % [N1 *L]
      case 2
      coil_ob = coil_ob(1:Nd(1),1:Nd(2),:); % [N1 N2 *L]
      case 3
      coil_ob = coil_ob(1:Nd(1),1:Nd(2),1:Nd(3),:); % [N1 N2 N3 *L]
      case 4
      coil_ob = coil_ob(1:Nd(1),1:Nd(2),1:Nd(3),1:Nd(4),:); % [N1 N2 N3 N4 *L]
      otherwise
      error 'only up to 4D implemented'
      end

      % scaling factors
      coil_ob = reshape(coil_ob, [prod(Nd) Lprod]);		% [*Nd *L]
      snc = conj(col(nufft_ob.arg.st.sn));				% [*Nd 1]
      coil_ob = coil_ob .* snc(:,ones(1,Lprod));			% scaling factors
      coil_ob = reshape(coil_ob, [Nd dims(2:end)]);		% [(Nd) (L)]
      tmp = nufft_ob.arg.mask .* coil_ob;
      x = x + conj(arg.sens(:,cc)) .* tmp(:);
    end
  end
else % approximation
  if isempty(arg.sens)
...
% full adjoint ("back-projection")
%
function x = Gmri_pback(arg, y, coils)
nc = ncol(y);
Nt = arg.dim(1);
Nr = length(coils);
y = y .* double(repmat(conj(arg.basis.transform), [1 nc]));
...
if isempty(arg.zmap)
  if isempty(arg.sens)
    ...
    for cc=1:Nr
      idcc = coils(cc);
      x = x + conj(arg.sens(:,idcc)) .* (arg.Gnufft' * y((cc-1)*Nt+1:cc*Nt));
    end
  end
else % approximation
  if isempty(arg.sens)
...
%		x = x + conj(arg.Ct(:,ll)) .* (arg.Gnufft' * (conj(arg.B(:,ll)) .* y));
    ...
    for ll=1:arg.L
      for cc=1:Nr
        ...
        tmp = repmat(conj(arg.B(:,ll)), [1 nc]) .* y((cc-1)*Nt+1:cc*Nt);
        tmp = arg.Gnufft' * tmp;
        tmp = repmat(conj(arg.sens(:,idcc)).*conj(arg.Ct(:,ll)), [1 nc]) .* tmp;
        x = x + tmp;
      end
    end
  end
