...
% self defined spiral traj: By Vincent Mao 1/31/2018, 
% Need to add random sampling
case 'spiral_VM'
	if fov(1) ~= fov(2) || N(1) ~= N(2), error 'only square done', end
    sampfactor = arg_traj{end};
    % choose r(t) = pi/6 t
    % achieve the spatial resolution, 0.75cm, AFe = 1, FOV = 24cm
    Nx = floor(fov(1) ./ (0.75 * sqrt(1/sampfactor) * cos(pi/6)));
    Ny = floor(fov(2) ./ (0.75 * sqrt(1/sampfactor) * sin(pi/6)));
    Nspiral = floor(prod([Nx Ny]));
	[kspace omega] = mri_kspace_spiral('N', max(N(1:2)), 'Nt', Nspiral ,'dt', 4e-6, 'gamp', 5, 'gslew', 200, ...
				'fov', max(fov(1:2)));
